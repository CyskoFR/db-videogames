...
-- 11- Selezionare il numero e la media delle recensioni per il videogioco con ID = 412 (review number = 12, avg_rating = 3)
	SELECT COUNT(videogame_id) AS 'review number'
	FROM reviews
...
-- 12- Selezionare il numero di videogame che la software house con ID = 1 ha rilasciato nel 2018 (13)
	SELECT COUNT(id) AS 'numero di videogame rilasciati nel 2018'
	FROM videogames
...
-- 1- Selezionare i dati di tutti i giocatori che hanno scritto almeno una recensione, mostrandoli una sola volta (996)
	SELECT name, lastname, nickname, city
	FROM players
	INNER JOIN reviews
	ON reviews.player_id = players.id
	GROUP BY name, lastname, nickname, city;
...
-- 3- Mostrare le categorie di ogni videogioco (1718)
	SELECT categories.id, categories.name, videogames.id, videogames.name
	FROM videogames
...
-- 5- Selezionare i premi ricevuti da ogni software house per i videogiochi che ha prodotto (55)
	...
	GROUP BY software_houses.name, awards.name, videogames.name;
...
-- 11- Selezionare i dati del videogame (id, name, release_date, totale recensioni) con più recensioni (videogame id : 398)
	SELECT TOP 1 videogames.id, name, release_date, COUNT(reviews.id) AS "totale recensioni"
	FROM videogames
	JOIN reviews
	ON videogames.id = reviews.videogame_id
	GROUP BY videogames.id, name, release_date
...
-- 13- Selezionare le categorie dei videogame i quali hanno una media recensioni inferiore a 1.5 (10)
	SELECT DISTINCT categories.name AS "category_name"
	FROM videogames
	JOIN reviews
	ON videogames.id = reviews.videogame_id
	JOIN category_videogame 
